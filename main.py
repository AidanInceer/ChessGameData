...
def extract_game_data(request: flask.Request):
    ...
    # setup environment:
    log = create_logger()
    config = Parser(request, log).parse_request()

    # Extract games data
    extractor = ExtractGameData(log, config)
    ...
    for game_num, game_pgn in enumerate(data):
        tud = TransformUserData(log, config)
        data, file_name = tud.create_gcs_dict_object(game_num, game_pgn)
        log.info(f"File {game_num} uploaded to bucket: {BUCKET}")
